...
def insert_row(table_name, upc, name, category, store_number, price, description,
       taxable, department, image):
    engine.execute(
        'INSERT INTO "%s" (upc, name, category, store_number, price, description, taxable, department, image) VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s" )'
        % (table_name, upc, name, category, store_number, price, description,
       taxable, department, image)
    )
...
def query_function(bucket, key, upc, store_num):
    ...
    df = df[(df['upc'] == str(upc)) & (df['store_number'] == str(store_num))]
...
#desirable too. Not enough time obviously to incorporate these. Have not extensively tested the code but hopefully,
# The logic is sound.